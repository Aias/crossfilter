...
describe('select', () => {
	describe('heapselect', () => {
		...
		it('can select from a small array of positive integers', function () {
			...
			assert.deepStrictEqual(select(array, 0, array.length, 2).sort(descending), [8, 7]);
			assert.deepStrictEqual(select(array, 0, array.length, 3).sort(descending), [8, 7, 6]);
			assert.deepStrictEqual(select(array, 0, array.length, 4).sort(descending), [8, 7, 6, 5]);
			assert.deepStrictEqual(select(array, 0, array.length, 5).sort(descending), [8, 7, 6, 5, 4]);
			assert.deepStrictEqual(select(array, 0, array.length, 6).sort(descending), [8, 7, 6, 5, 4, 3]);
			assert.deepStrictEqual(select(array, 0, array.length, 7).sort(descending), [8, 7, 6, 5, 4, 3, 2]);
			assert.deepStrictEqual(select(array, 0, array.length, 8).sort(descending), [8, 7, 6, 5, 4, 3, 2, 1]);
		});
